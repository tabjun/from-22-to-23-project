# -*- coding: utf-8 -*-
"""
Created on Sat Aug 20 19:20:57 2022
...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug 20 18:34:35 2022
...
#%%
#numberoftrips범주화
tr['num_gr'] = tr['numberoftrips'].apply(lambda x: '4회 미만' if x < 4 else '4회 이상' if 4 <= x else x) 
print(tr[tr['num_gr']=='4회 이상']['numberoftrips'])
print(tr[tr['num_gr'].isnull()==True]['numberoftrips'])
print(tr['num_gr'].value_counts())
#%%
tr['num_gr'] = tr['num_gr'].fillna('4회 이상')
print(f"pitch결측치:{tr['num_gr'].isnull().sum()}");print('\t')
print(tr['num_gr'].value_counts())
#%%
...
print(tr['group_pitch_10'].value_counts())
#%%
tr['group_pitch_5'] = tr['group_pitch_5'].fillna('5이상 10미만')
print(f"pitch결측치:{tr['group_pitch_5'].isnull().sum()}");print('\t')
print(tr['group_pitch_5'].value_counts())
...
tr['monthlyincome'] = tr['monthlyincome'].fillna(tr.groupby(['owncar','gender',"designation",'numberoftrips'])['monthlyincome'].transform('mean'))
#%%
...
print(anova_lm(model))
#%%
tr['age'] = tr['age'].fillna(tr.groupby(['owncar',"designation",'numberoftrips','maritalstatus','productpitched'])['age'].transform('mean'))
#%%
...
# 일단 로버스트 정규화로 돌려보기
#%%
tr = pd.get_dummies(tr,columns=['occupation'])
#%%
#라벨 인코딩을 위한 문자형 변수만 추출
object_columns = tr.columns[tr.dtypes == 'object']
print('object 칼럼은 다음과 같습니다 : ', list(object_columns))
#%%
exp_clf = setup(data = tr, target = 'prodtaken',fold_shuffle = True,normalize=True,normalize_method='robust',
                session_id=1234,ignore_features=['id','numberoftrips','monthlyincome','occupation_Small Business'])
#%%
#정확도기준 모델 형성 및 성능 비교
...
test['group_pitch_5'] = test['durationofpitch'].apply(lambda x: '5이상 10미만' if 5 <= x < 10 else '10이상 15미만' if 10 <= x < 15 else '15이상 20미만' if 15 <= x < 20 else '20이상 25미만' if 20 <= x < 25 else '25이상 30미만' if 25 <= x < 30 else '30이상 35미만' if 30 <= x < 35  else '35이상' if 35 <= x else x) 
#%%
#numberoftrips범주화
test['num_gr'] = test['numberoftrips'].apply(lambda x: '4회 미만' if x < 4 else '4회 이상' if 4 <= x else x) 
print(test[test['num_gr']=='4회 이상']['numberoftrips'])
print(test[test['num_gr'].isnull()==True]['numberoftrips'])
print(test['num_gr'].value_counts())
#%%
test['num_gr'] = test['num_gr'].fillna('4회 이상')
print(f"pitch결측치:{test['num_gr'].isnull().sum()}");print('\t')
print(test['num_gr'].value_counts())
#%%
...
print(test['typeofcontact'].isnull().sum())
#%%
...
print(test.isnull().sum())
#%%
print(test['group_pitch_5'].value_counts())
#%%
# durationofpitch결측치 처리
test['group_pitch_5'] = test['group_pitch_5'].fillna('5이상 10미만')
print(f"pitch결측치:{test['group_pitch_5'].isnull().sum()}");print('\t')
print(test['group_pitch_5'].value_counts())
#%%
...
#%%
#모델 튜닝
tuned_rf = tune_model(rf,fold = 3 , optimize='Accuracy',choose_better=True)
tuned_et = tune_model(et,fold = 3 , optimize='Accuracy',choose_better=True)
#%%
#엑스트라 트리
final_et = finalize_model(tuned_et)
pred_et = predict_model(final_et, data = test)
#%%
# 랜포 prec 1등
...
final_et = finalize_model(tuned_et)
#%%
pred_et.to_csv('C:\\Users\\215-05\\Desktop\\여행여부 예측\\et_drop_num_gr.csv',
               columns = ['Label'])
#%%
