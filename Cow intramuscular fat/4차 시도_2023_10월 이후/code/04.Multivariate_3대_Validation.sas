...
proc logistic data=TRAINSET plots=ROC outest=betas;
...
model TARGET1 (event='1') =  개월령 생시체중 근내평균 도체범위근내평균 체고 체장
												...
												농가구분/ outroc=troc;
score data=validset out=valpred outroc=vroc;
roc; roccontrast;
run;
...
ods listing close;
ods pdf file='C:\Users\Owner\Desktop\윤태준\소\231024_Analysis\result\full농가제거mul.pdf';
...
ods listing close;
ods excel file='C:\Users\Owner\Desktop\윤태준\소\231024_Analysis\result\mul_변수제거.xlsx';

/* d*/
proc logistic data=TRAINSET plots=ROC outest=betas;
class 출산여부_6개월내 (ref='Y') 농가구분 (ref='농가소');
model TARGET1 (event='1') =    근내평균 도체범위근내평균  
												 형매근내평균가산
												출산여부_6개월내  농가근내평균가산 근내EPD
												농가구분/ outroc=troc;
score data=validset out=valpred outroc=vroc;
roc; roccontrast;
run;
...
ods listing close;
ods pdf file='C:\Users\Owner\Desktop\윤태준\소\231024_Analysis\result\mul_변수제거.pdf';
...
/* firth 보정 */
* A_S_C A_S_F_M B_S_M_W B_S_M_I B_S_F_M B_S_C제외 (p-value 0.2 기준) - AUC 0.743 - 최종 모형 확인 ;
...
/* target 789 */
PR = -3.2682 + (-0.0365*SEX) + (1.3856 *F1) + (-0.3213*F2) + (-0.0073*SL_M) + (0.00711*S_W) 
		+ (-0.00176*S_M_W) + (-0.00701*S_F_M) + (0.1691*S_T_M) 
		+ (-0.0162 * S_C) +(-0.00095*A_S_M_W) + (0.0881*A_S_T_M) 
		+ (0.00008*B_S_M_W) + (0.00198*B_S_M_I) + (0.0327*B_S_T_M);
...
/* target only 9 */
PR_2 = -5.7754 + (0.0872*SEX) + (1.2462 *F1) + (-0.1686*F2) + (-0.0108*SL_M) + (0.00792*S_W) 
		+ (-0.00233*S_M_W) + (-0.0035*S_M_I) + (-0.0133*S_F_M) + (0.1996*S_T_M) 
		+ (-0.00166*A_S_M_W) + (0.00427*A_S_M_I) + (0.112*A_S_T_M)
		+ (0.0317*B_S_T_M);
...
BY DESENDING PREDP;
RUN;
PROC PRINT DATA= SORTED_1 (OBS=56);
RUN;

PROC EXPORT DATA=SORTED_1
OUTFILE = 'C:\Users\Owner\Desktop\윤태준\소\3차 요청자료\230301_Analysis\SORTED_TARGET_789_ONLY_PROBA.xlsx'
dbms=xlsx replace;
run;
...
RUN;
PROC PRINT DATA= SORTED_2 (OBS=56);
RUN;

PROC EXPORT DATA=SORTED_2
OUTFILE = 'C:\Users\Owner\Desktop\윤태준\소\3차 요청자료\230301_Analysis\SORTED_TARGET_9_ONLY_PROBA.csv'
dbms=csv replace;
run;
