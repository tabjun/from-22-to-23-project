/*  SAS Dataset 불러오기 */
libname COW 'C:\Users\Owner\Desktop\윤태준\소\3차 요청자료\230301_Analysis\age조절';

data RAW; set COW.RAW; run;
data TESTSET; set COW.TESTSET; run;
data VALIDSET; set COW.VALIDSET; run;

* S_M_I S_C A_S_M_I A_S_C B_S_F_M B_S_C 제외 (p-value 0.2 기준) - AUC 0.695 - 최종 모형 확인 ;
...
model TARGET1 (event='1') =  GENDER FARM_LEVEL SL_M S_W
                                               S_M_W S_F_M S_T_M
                                               A_S_M_W A_S_F_M A_S_T_M  
											   B_S_M_W B_S_M_I B_S_T_M ;
run;

* A_S_F_M B_S_M_W B_S_M_I B_S_F_M 제외 (p-value 0.2 기준) - AUC 0.724 - 최종 모형 확인 ;
...
/* target 789 */
PR = -3.74908 + (-0.03778*SEX) + (1.27507 *F1) + (-0.27284*F2) + (-0.01292*SL_M) + (0.00654*S_W) 
		+ (-0.00192*S_M_W) + (-0.00620*S_F_M) + (0.17393*S_T_M) 
		+ (-0.00097*A_S_M_W) + (-0.00398*A_S_F_M) + (0.09007*A_S_T_M) 
		+ (0.00054*B_S_M_W) + (0.00259*B_S_M_I) + (0.02728*B_S_T_M);
...
/* target only 9 */
PR_2 = -6.431 + (0.0949*SEX) + (1.2559 *F1) + (-0.1695*F2) + (-0.0124*SL_M) + (0.0077*S_W) 
		+ (-0.0022*S_M_W) + (-0.0044*S_M_I) + (-0.0129*S_F_M) + (0.1896*S_T_M) 
		+ (-0.0020*A_S_M_W) + (0.0044*A_S_M_I) + (0.1187*A_S_T_M)
		+ (0.0401*B_S_T_M);
...
OUTFILE = 'C:\Users\Owner\Desktop\윤태준\소\3차 요청자료\230301_Analysis\age조절\sorted_789_result.csv'
dbms=csv replace;
run;
...
OUTFILE = 'C:\Users\Owner\Desktop\윤태준\소\3차 요청자료\230301_Analysis\age조절\sorted_9_result.csv'
dbms=csv replace;
run;
