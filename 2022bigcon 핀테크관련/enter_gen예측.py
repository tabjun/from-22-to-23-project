...
#%%
train_enter = pd.read_csv('train_gen_enter_sca.csv')
#%%
test_enter = pd.read_csv('test_gen_enter_sca.csv')
# In[6]:
final = pd.get_dummies(train_enter,columns = [ 'income_type',
...
#%%
dum_test = pd.get_dummies(test_enter,columns = [ 'income_type',
'employment_type', 'houseown_type', 'purpose'])
#%%
x_test = dum_test.drop(['Unnamed: 0', 'is_applied','houseown_type_자가'],axis=1)
#%%
test_enter_predict = xgb1.predict(x_test)
#%%
a1= pd.DataFrame(test_enter_predict)
#%%
test_enter.is_applied = a1
#%%
test_enter.to_csv('gen_enter예측완_cp.csv',encoding = 'cp949')
test_enter.to_csv('gen_enter예측완.csv')
