...
print(df.isnull().sum())
#%%
# 85개 결측에 해당하는 user_id를 추출, isin을 사용하여 해당하는 데이터프레임 생성
a = df[df['employment_type'].isnull()==True]['user_id'].tolist()
em_null = df_copy[df_copy['user_id'].isin(a)]
em_null.sort_values(['user_id'],inplace =True)
em_null = em_null[['application_id','user_id','insert_time','employment_type','대출신청여부']]
#%%
pdf_most = pd.pivot_table(df,                # 피벗할 데이터프레임
                     index = 'user_id',    # 행 위치에 들어갈 열
                     values = 'employment_type',     # 데이터로 사용할 열
                     aggfunc = 'top')   # 데이터 집계함수
pdf_most
#%%
df_copy['employment_type'].fillna(em_null.groupby(['user_id'])['employment_type'].mode)
