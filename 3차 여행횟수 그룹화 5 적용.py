...
#%%
#numberoftrips범주화
tr['num_gr'] = tr['numberoftrips'].apply(lambda x: '5회 미만' if x < 5 else '5회 이상' if 5 <= x else x) 
print(tr[tr['num_gr']=='5회 이상']['numberoftrips'])
print(tr[tr['num_gr'].isnull()==True]['numberoftrips'])
print(tr['num_gr'].value_counts())
#%%
tr['num_gr'] = tr['num_gr'].fillna('5회 이상')
...
exp_clf = setup(data = tr, target = 'prodtaken',fold_shuffle = True,normalize=True,normalize_method='robust',
                session_id=1234,ignore_features=['id','numberoftrips'])
#%%
#정확도기준 모델 형성 및 성능 비교
...
#%%
#numberoftrips범주화
test['num_gr'] = test['numberoftrips'].apply(lambda x: '5회 미만' if x < 5 else '5회 이상' if 5 <= x else x) 
print(test[test['num_gr']=='5회 이상']['numberoftrips'])
print(test[test['num_gr'].isnull()==True]['numberoftrips'])
print(test['num_gr'].value_counts())
#%%
test['num_gr'] = test['num_gr'].fillna('5회 이상')
...
final_et = finalize_model(tuned_et)
#%%
pred_et.to_csv('C:\\Users\\215-05\\Desktop\\여행여부 예측\\et_robust_num_gr.csv',
               columns = ['Label'])
#%%
