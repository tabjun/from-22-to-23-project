...
plt.show()
#%%
tr['monthlyincome'] = np.log1p(tr['monthlyincome'])
#%%
tr['monthlyincome'] = np.exp(tr['monthlyincome'])
#%%
sns.distplot(tr.monthlyincome)
plt.show()
#%%
tr_copy = tr.copy()
tr_copy = tr_copy.astype({'typeofcontact':'category','citytier':'category','numberofpersonvisiting':'category',
                          'numberoftrips':'category','passport':'category','pitchsatisfactionscore':'category',
                          'owncar':'category','numberofchildrenvisiting':'category'})
...
#%%
#모델 형성 및 성능 비교
top5_model = compare_models(round=4, n_select = 5, sort='Accuracy')
...
#%%
print(test[test['numberoftrips']==2].describe(include='object'))
